...
namespace Nethereum.Contracts.IntegrationTests.FiltersEvents
{
    ...
    public class EventFilterTopic
    {
        ...
        public async Task StoreAndRetrieveStructs()
        {
            ...
            var receipt1 = await storeDocumentFunction.SendTransactionAndWaitForReceiptAsync(account.Address, new HexBigInteger(900000), null, CancellationToken.None, "k1", "doc1", "Document 1");
            Assert.Equal(1, receipt1.Status?.Value);
            var receipt2 = await storeDocumentFunction.SendTransactionAndWaitForReceiptAsync(account.Address, new HexBigInteger(900000), null, CancellationToken.None, "k2", "doc2", "Document 2");
        ...
        public async Task DeployAndCallContract_WithEvents()
        {
            ...
            var receipt1 = await multiplyFunction.SendTransactionAndWaitForReceiptAsync(account.Address, new HexBigInteger(estimatedGas.Value), null, CancellationToken.None, 5);
            var receipt2 = await multiplyFunction.SendTransactionAndWaitForReceiptAsync(account.Address, new HexBigInteger(estimatedGas.Value), null, CancellationToken.None, 7);
